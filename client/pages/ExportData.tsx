...
export default function ExportData() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Select all major sections to potentially split them
    ...
    <div className="min-h-screen flex flex-col bg-muted/30">
      ...
      <main className="flex-1 py-12 px-4 md:px-6">
        <div className="container max-w-4xl">
          ...
          <Card className="p-0 shadow-sm bg-transparent border-none overflow-hidden" ref={reportRef}>
            {/* Page 1: Summary & Topics */}
            <div className="report-section p-8 bg-white mb-4 rounded-xl border border-slate-100">
              {/* Report Header */}
                ...
                <div className="space-y-8">
                  {["Pure", "Statistics", "Mechanics"].map(category => {
                    const categoryTopics = stats?.topics.filter(t => {
                      ...
                      return chapter?.category === category || (category === "Pure" && !chapter);
                    }) || [];
            ...
            {/* Page 2: Detailed Paper Breakdown */}
            <div className="report-section p-8 bg-white rounded-xl border border-slate-100 min-h-[1000px]">
              <h2 className="text-2xl font-bold text-slate-900 mb-8 flex items-center gap-2 border-b pb-4">
                  ...
                  <div key={paper.submission_id} className="bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {paper.questions.map((q) => (
                        <div key={q.question_number} className="bg-white p-3 rounded-lg border border-slate-100 flex justify-between items-center shadow-sm">
